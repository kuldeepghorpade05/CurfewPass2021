...
$sql="insert into tblhospitalpass(PassNumber,FullName,ContactNumber,Email,IdentityType,IdentityCardno,HospitalName,Reason,FromDate,ToDate)values(:passnum,:fname,:cnum,:email,:itype,:icnum,:cat,:reason,:fdate,:tdate)";
...
echo "<script>window.location.href ='add-hospital-pass.php'</script>";
  }
  else
    {
...
<head>
    ...
    <title>Curfew Pass Management System | Add Hospital Pass</title>
        ...
        <!--  page-wrapper -->
          <div id="page-wrapper">
            <div class="row">
                 <!-- page header -->
                <div class="col-lg-12">
                    <h1 class="page-header">Generate Hospital Visit Pass</h1>
                </div>
                <!--end page header -->
            </div>
            <div class="row">
...
{          
    ?>  
<option value="<?php echo htmlentities($row->HospitalName);?>"><?php echo htmlentities($row->HospitalName);?></option>
 <?php } ?>
     </select></div>
	 <div class="form-group"> <label for="exampleInputEmail1">Reason to visit Hospital</label> <input type="text" name="reason" value="" class="form-control" required='true'> </div>
